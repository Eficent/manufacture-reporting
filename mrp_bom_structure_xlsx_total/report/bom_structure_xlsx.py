...
class BomStructureXlsxTotal(BomStructureXlsx):


    def print_bom_children(self, bom, requirements, sheet, row):
        i = row
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        def _compute_totals(bom, factor=1):
            factor *= bom.product_qty
            for line in bom.bom_line_ids:
                sub_bom = bom._bom_find(product=line.product_id)
                if sub_bom:
                    _compute_totals(sub_bom, factor)
                else:
                    if totals.get(line.product_id):
                        totals[line.product_id] += line.product_uom_id._compute_quantity(
        ...
        for o in objects:
            totals = {}
            _compute_totals(o)
            i = self.print_bom_children(o, totals, sheet, i)


BomStructureXlsxTotal('report.bom.structure.xlsx.total', 'mrp.bom',
